import { ProjectCover } from "@/components/project"
import { PROJECTS } from "@/config/content";
import { ProjectPreview } from "@/config/types";
import { Metadata } from "next";

const categories = Array.from(
  new Set(PROJECTS.map(p => p.category).filter(Boolean))
);

const grouped: [string, ProjectPreview[]][] = Object.entries<ProjectPreview[]>({
  ...Object.fromEntries(
    categories.map(cat => [cat, PROJECTS.filter(p => p.category === cat)])
  ),
  Uncategorized: PROJECTS.filter(p => !p.category),
}).filter(([_, projects]) => projects.length > 0) // del. empty categories

export const metadata: Metadata = {
  title: "All Projects",
  description: "Explore my software engineering portfolio featuring full-stack projects, product design case studies, startup builds, and production-ready applications.",
}

export default function Page() {
  return (
    <section className="min-h-screen bg-neutral-100">
      <div className="px-8 py-8 space-y-8 max-w-7xl mx-auto">
        <div className="space-y-1">
          <h1 className="text-3xl font-semibold">All Projects</h1>
          <p className="text-sm">Explore my software engineering portfolio featuring full-stack projects, product design case studies, startup builds, and production-ready applications.</p>
        </div>

        {grouped.map(([category, projects]) => (
          <div key={category} className="space-y-4">
            <h2 className="text-xl font-semibold">{category}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {projects.map((p, i) => (
                <ProjectCover key={`${category}-${i}`} project={p}/>
              ))}
            </div>
          </div>
        ))}
      </div>
    </section>
  );
}
